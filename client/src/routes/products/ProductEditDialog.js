...
class ProductEditDialog extends React.Component {
  ...
  onSubmit = (id) => async (data) => {
    data.id = id;
    await this.props.productUpdate({ variables: { data } });
    ...
    <form onSubmit={handleSubmit}>
      ...
      <Dialog.Body scrollable>
        <Grid.Layout gap="sm" stretch>
          ...
          <Grid.Box>
            <Field name="description" label="Description" type="textarea" placeholder="Description" component={InputField} />
          </Grid.Box>
          <Grid.Box>
            <Field name="price" label="Price" type="number" placeholder="Price" component={InputField} />
          </Grid.Box>
          <Grid.Box>
  ...
  renderForm = ({ args }) => {
    return (
      <Form type="UPDATE" tableSchemaName="Products" onSubmit={this.onSubmit(args.initialValues.id)} initialValues={args.initialValues}>
